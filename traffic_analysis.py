    ...
    try:
        with open(log_file_path, 'r', encoding='utf-8') as csvfile:
            ...
            for row_num, row in enumerate(reader):
                total_requests += 1
                # Extract only essential fields for current rules + a few counters
                request_data = {field: row.get(field) for field in ESSENTIAL_TRAFFIC_FIELDS | {'request_method', 'server_protocol', 'time', 'user_id'}}
                # If you need ALL fields later, use:
                # request_data = {field: row.get(field) for field in TRAFFIC_LOG_FIELDS}
                ...
                for rule in anomaly_rules:
                    ...
                    if 'pattern' in rule:
                        ...
                        if field_to_check and re.search(rule['pattern'], str(field_to_check), re.IGNORECASE):
                             ...
                             # anomaly_details['matched_value'] = match.group(0) if match else 'N/A' # Optional detail
                    ...
                    if triggered:
                        ...
                        # break # Optional: only report first matching rule per row
